...
impl TaskExecutor {
    ...
    /// # Panics
    ///
    /// Panics if the corresponding `TaskRunner` has disappeared.
    ...
    /// Executes a future, but as a *daemon*.
    ///
    ...
    ///
    /// Panics if the corresponding `TaskRunner` has disappeared.
    pub fn spawn_daemon<F>(&self, future: F)
        where F: Future<Item = (), Error = ()> + 'static,
    {
    ...
    where F: Future<Item = (), Error = ()> + 'static,
{
    match inner.upgrade() {
        ...
        None => panic!("corresponding task runner has disappeared"),
    }
}
