...
/// Futures managed by [`FuturesKeyed`] will only be polled when they
/// generate wake-up notifications. This reduces the required amount of work
/// needed to poll large numbers of futures.
///
/// [`FuturesKeyed`] can be filled by [`collect`](Iterator::collect)ing an
/// iterator of futures into a [`FuturesKeyed`], or by
/// [`push`](FuturesKeyed::push)ing futures onto an existing
/// [`FuturesKeyed`]. When new futures are added,
/// [`poll_next`](Stream::poll_next) must be called in order to begin receiving
/// wake-ups for new futures.
///
/// Note that you can create a ready-made [`FuturesKeyed`] via the
/// [`collect`](Iterator::collect) method, or you can start with an empty set
/// with the [`FuturesKeyed::new`] constructor.
///
...
// FuturesKeyed is implemented using two linked lists. One which links all
// futures managed by a `FuturesKeyed` and one that tracks futures that have
...
// `FuturesKeyed` value for any other operations. The second linked list is
...
// `FuturesKeyed`. When a wake-up notification is received, the task is
...
impl<K, Fut, S: ReleasesTask<K>> FuturesKeyed<K, Fut, S> {
    ...
    fn release_task(&mut self, task: Arc<Task<K, Fut>>) {
        ...
        // `FuturesKeyed`, which correctly tracks `Fut`'s lifetimes and
        // such.
        unsafe {
    ...
    ///
    /// `FuturesKeyed::link` needs to update `Task` pointers in an order
    ...
    fn pending_next_all(&self) -> *mut Task<K, Fut> {
        ...
        // this `FuturesKeyed`, so we can make use of its value here.
...
impl<K, Fut, S: ReleasesTask<K>> Drop for FuturesKeyed<K, Fut, S> {
    fn drop(&mut self) {
        ...
        // this point the `FuturesKeyed` struct, the owner of the one strong
