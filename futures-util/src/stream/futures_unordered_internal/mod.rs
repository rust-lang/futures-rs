...
/// This replaces futures_unordered as the main runner of unordered futures
/// The same futures_unordered api will now be a consumer of this module.
/// This module is changed from futures_unordered only in that Task structs contain a hashable key,
/// and that Task will be returned with its future's output when each future completes.
/// The whole Task must be returned, since we don't want the Key to need to be Copy or Clone, and
/// so the Key that is used to track the tasks in the consuming module must be the same instance of
/// the Key that is contained in the returned Task.
...
/// Futures managed by [`FuturesUnorderedInternal`] will only be polled when they
/// generate wake-up notifications. This reduces the required amount of work
/// needed to poll large numbers of futures.
///
/// [`FuturesUnorderedInternal`] can be filled by [`collect`](Iterator::collect)ing an
/// iterator of futures into a [`FuturesUnorderedInternal`], or by
/// [`push`](FuturesUnorderedInternal::push)ing futures onto an existing
/// [`FuturesUnorderedInternal`]. When new futures are added,
/// [`poll_next`](Stream::poll_next) must be called in order to begin receiving
/// wake-ups for new futures.
///
/// Note that you can create a ready-made [`FuturesUnorderedInternal`] via the
...
pub(super) struct FuturesUnorderedInternal<K, Fut, S: ReleasesTask<K>> {
...
// `FuturesUnorderedInternal` value for any other operations. The second linked list is
...
// `FuturesUnorderedInternal`. When a wake-up notification is received, the task is
...
impl<K, Fut, S: ReleasesTask<K>> Default for FuturesUnorderedInternal<K, Fut, S> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<K, Fut, S: ReleasesTask<K>> FuturesUnorderedInternal<K, Fut, S> {
    /// Constructs a new, empty [`FuturesUnorderedInternal`].
    ///
    /// The returned [`FuturesUnorderedInternal`] does not contain any futures.
    /// In this state, [`FuturesUnorderedInternal::poll_next`](Stream::poll_next) will
    ...
    fn release_task(&mut self, task: Arc<Task<K, Fut>>) {
        ...
        // `FuturesUnorderedInternal`, which correctly tracks `Fut`'s lifetimes and
        // such.
        unsafe {
    ...
    /// managed by `FuturesUnorderedInternal`.
    ...
    ///
    /// `FuturesUnorderedInternal::link` needs to update `Task` pointers in an order
    ...
    fn pending_next_all(&self) -> *mut Task<K, Fut> {
        ...
        // this `FuturesUnorderedInternal`, so we can make use of its value here.
        Arc::as_ptr(&self.ready_to_run_queue.stub) as *mut _
    }
}

impl<K, Fut: Future, S: ReleasesTask<K>> Stream for FuturesUnorderedInternal<K, Fut, S> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            struct Bomb<'a, K, Fut, S: ReleasesTask<K>> {
                queue: &'a mut FuturesUnorderedInternal<K, Fut, S>,
...
impl<K, Fut, S: ReleasesTask<K>> Debug for FuturesUnorderedInternal<K, Fut, S> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "FuturesUnorderedInternal {{ ... }}")
    }
}
...
impl<K, Fut, S: ReleasesTask<K>> Drop for FuturesUnorderedInternal<K, Fut, S> {
    fn drop(&mut self) {
        // When a `FuturesUnorderedInternal` is dropped we want to drop all futures
        ...
        // this point the `FuturesUnorderedInternal` struct, the owner of the one strong
...
impl<'a, K, Fut: Unpin, S: ReleasesTask<K>> IntoIterator
    for &'a FuturesUnorderedInternal<K, Fut, S>
{
    type Item = (&'a K, &'a Fut);
...
impl<'a, K, Fut: Unpin, S: ReleasesTask<K>> IntoIterator
    for &'a mut FuturesUnorderedInternal<K, Fut, S>
{
...
impl<K, Fut: Unpin, S: ReleasesTask<K>> IntoIterator for FuturesUnorderedInternal<K, Fut, S> {
    type Item = (K, Fut);
...
impl<K, Fut, S: ReleasesTask<K>> FromIterator<(K, Fut)> for FuturesUnorderedInternal<K, Fut, S> {
    fn from_iter<I>(iter: I) -> Self
    where
        I: IntoIterator<Item = (K, Fut)>,
    {
        let acc = Self::new();
        iter.into_iter().fold(acc, |acc, (key, fut)| {
            acc.push(key, fut);
            acc
        })
    }
...
impl<K, Fut: Future, S: ReleasesTask<K>> FusedStream for FuturesUnorderedInternal<K, Fut, S> {
    fn is_terminated(&self) -> bool {
        self.is_terminated.load(Relaxed)
    }
}

impl<K, Fut, S: ReleasesTask<K>> Extend<(K, Fut)> for FuturesUnorderedInternal<K, Fut, S> {
