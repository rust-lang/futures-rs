...
impl WakerKey {
    ...
    pub(crate) const NULL: WakerKey = WakerKey(usize::MAX);
}
...
impl WakerSet {
    ...
    pub(crate) fn record_waker(&self, key: &mut WakerKey, cx: &mut Context<'_>) {
        let mut wakers_guard = self.wakers();
    ...
    #[cfg(feature = "std")]
    fn wakers(&self) -> std::sync::MutexGuard<'_, Option<Slab<Option<Waker>>>> {
        self.wakers.lock().unwrap()
    }

    #[cfg(not(feature = "std"))]
    fn wakers(&self) -> spin::mutex::MutexGuard<'_, Option<Slab<Option<Waker>>>> {
        self.wakers.lock()
    }
}
