//! Asynchronous stream object value
...
/// Stealing code from core::future::future_obj
pub struct LocalStreamObj<'a, T> {
    ptr: *mut (),
    poll_next_fn: unsafe fn(*mut (), &mut Context) -> Poll<Option<T>>,
...
pub unsafe trait UnsafeStreamObj<'a, T>: 'a {
    ...
    /// `drop`.
    unsafe fn poll_next(ptr: *mut (), cx: &mut Context) -> Poll<Option<T>>;
