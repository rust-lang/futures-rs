          ...
          T::SinkError: From<U::Error>,
{
    ...
    fn try_start_send(&mut self, task: &Task, item: U::Item) -> Poll<(), T::SinkError> {
        debug_assert!(self.buffered.is_none());
        if let AsyncSink::NotReady(item) = try!(self.sink_mut().start_send(task, item)) {
          ...
          T::SinkError: From<U::Error>,
{
    ...
    fn poll(&mut self, task: &Task) -> Poll<(T, U), T::SinkError> {
        ...
        loop {
            match try!(self.stream_mut().poll(task)) {
